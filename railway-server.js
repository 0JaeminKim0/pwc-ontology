...
function loadContent() {
  ...
  // 인라인 HTML도 로드 (있다면)
  if (existsSync('dist/inline.html')) {
    try {
      const inlineHtml = readFileSync('dist/inline.html', 'utf8')
      content.set('/inline', inlineHtml)
      console.log(`✅ 인라인 HTML 로드됨 (${inlineHtml.length} 문자)`)
    } catch (error) {
      console.error('❌ 인라인 HTML 로드 실패:', error.message)
    }
  }
